import type { Express, Response as ExpressResponse, NextFunction } from "express";
import { ObjectStorageService, ObjectNotFoundError } from "./objectStorage";
import { isAuthenticated } from "../auth";
import { authStorage } from "../auth/storage";
import { storage } from "../../storage";

// Admin-only middleware. Looks up the logged-in user's role and rejects
// non-admins. Used to gate diagnostic endpoints that leak bucket metadata.
async function requireAdmin(req: any, res: ExpressResponse, next: NextFunction) {
  try {
...
// Allow a request through if the caller is logged in OR carries a valid
// presentation share-token (?pt=<token>) that matches a planning board's
// canvasData.shareToken. Used to gate /objects/* reads so the public
// `/p/:token` presentation flow keeps working without leaving the bucket
// open to anonymous traffic. The token is generated by an admin/crew member
// via /api/board/presentation-link and is per-board; revoking it (clearing
// shareToken) immediately denies access.
async function isAuthenticatedOrValidPresentationToken(
  req: any,
  res: ExpressResponse,
  next: NextFunction,
) {
  if (req.user) return next();
  const pt = typeof req.query?.pt === "string" ? req.query.pt : "";
  if (pt) {
    try {
      const projects = await storage.getProjects();
      for (const proj of projects) {
        const boards = await storage.getPlanningBoards(proj.id);
        if (boards.some((b: any) => (b.canvasData as any)?.shareToken === pt)) {
          return next();
        }
      }
    } catch {
...
export function registerObjectStorageRoutes(app: Express): void {
   ...
   *
   * Admin-only. The response echoes bucket names, which is leaky enough that
   * unauthenticated callers shouldn't see it.
   */
  app.get("/api/uploads/diagnostic", isAuthenticated, requireAdmin, async (_req, res) => {
   ...
   */
  app.post("/api/uploads/from-url", isAuthenticated, async (req, res) => {
    const { url } = req.body || {};
   ...
   */
  app.get("/objects/{*objectPath}", isAuthenticatedOrValidPresentationToken, async (req, res) => {
    try {
