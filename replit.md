...
*   **Gantt Chart / Progress Tab**: The "Progress" tab in ProjectDetails combines three sub-views: Timeline (Gantt chart), Checklist, and Calendar. The Gantt chart (`client/src/components/GanttChart.tsx`) uses a **progressive drill-down Scope view** with three levels: Phase → Section → Task. The left column header reads "SCOPE" at phase level, "SECTIONS" when drilled into a phase, and "TASKS" when viewing a section's tasks. Clicking a phase row drills into its sections; clicking a section drills into its tasks. Back arrow and breadcrumb navigation allow returning to parent levels. Tasks have checkboxes — checking/unchecking toggles status between "done" and "todo" via the existing PATCH /api/tasks/:id endpoint. Progress bars and completion counts update in real time. Each drill level adapts the Gantt timeline date range to the focused scope. Unsectioned tasks under a phase appear as a "General Tasks" group at section level. State reconciliation auto-resets drill level when selected phase/section is deleted. Phases (milestones) have `startDate`/`endDate` fields. Sections (`sections` table) group tasks under phases with their own date ranges. Tasks optionally link to sections via `sectionId`. Role-gated: clients see read-only, crew/admin can add phases/sections/tasks and check off tasks. Phase creation form removed from Overview tab (replaced with hint text directing to Timeline view). Schema: `sections` table (milestoneId FK with cascade delete, projectId, title, startDate, endDate, completed, order). API: `GET/POST /api/projects/:projectId/sections`, `PATCH/DELETE /api/sections/:id` with role-based authorization. Calendar is no longer a standalone top-level tab — it's accessed within Progress.
