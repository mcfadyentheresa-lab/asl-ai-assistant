...
- **AppShell + Sidebar Navigation (Task #52)**: All authenticated routes (excluding `/welcome` and `/invite/:token`) are wrapped in `AppShell` (`client/src/components/layout/AppShell.tsx`). AppShell renders the top `NavbarShell` + a `DesktopSidebar` (icon-rail at `md`, full labels at `lg`) + a Sheet-based mobile drawer triggered by a hamburger button. Role-aware navigation is driven by `client/src/lib/nav-config.ts` which defines `NAV_ITEMS[]` with `allowedRoles` and `group`. The `Navbar` export checks `AppShellContext` and returns null when inside AppShell (no double navbar). Nav groups: **Projects** (Dashboard; client also gets dynamic "Your Project" link), **Operations** (Calendar/Timesheets/Payroll/Colours/Crew&Trade), **Resources** (Suppliers/Furniture — admin only), **Growth** (Social Media — admin only), **Personal** (Profile). Mobile drawer uses shadcn `<Sheet>` component. Take the Tour and Text Size controls live in the sidebar footer (not the Navbar dropdown).
- **Navbar cleanup**: Avatar dropdown (`data-testid="button-user-menu"`) now contains only: Your Profile, Take the Tour, Text Size, Sign Out. All page navigation links (Calendar, Timesheets, Payroll, Colors, etc.) were removed from the dropdown — they now live exclusively in the sidebar.
- **Mobile/desktop adaptive banners**: Planning Board shows a "full editing on desktop" notice on `isMobileView`. CostEstimator, SupplierPrices, and Payroll show a subtle `md:hidden` "best on larger screen" banner without hiding content.
