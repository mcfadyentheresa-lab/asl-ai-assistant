...
- **AppShell + Sidebar Navigation (Task #52)**: All authenticated routes (excluding `/welcome` and `/invite/:token`) are wrapped in `AppShell` (`client/src/components/layout/AppShell.tsx`). AppShell renders the top `NavbarShell` + a `DesktopSidebar` (hidden on mobile) + a Sheet-based mobile drawer triggered by a hamburger button. Role-aware navigation is driven by `client/src/lib/nav-config.ts` which defines `NAV_ITEMS[]` with `allowedRoles` and `group`. The `Navbar` export checks `AppShellContext` and returns null when inside AppShell (no double navbar). Nav groups: Overview (all roles), Operations (admin+crew), Tools (admin), Personal (all). Mobile drawer uses shadcn `<Sheet>` component.
