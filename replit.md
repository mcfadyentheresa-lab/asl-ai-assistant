...
*   **Gantt Chart / Progress Tab (Buildings → Rooms Nested Timeline)**: The "Progress" tab in ProjectDetails combines three sub-views: Timeline (Gantt chart), Checklist, and Calendar. The Gantt chart (`client/src/components/GanttChart.tsx`) uses a **nested expandable tree** with two drill levels. Level 1 (buildings view): Buildings (mapped to milestones DB table) are top-level rows with expand/collapse chevrons; Rooms (mapped to sections DB table) are shown indented below each building. Both building and room Gantt bars display simultaneously on the timeline. Clicking a room drills into Level 2 (tasks view): tasks assigned to that room. Breadcrumb: All Buildings > Building > Room. Buildings have `startDate`/`endDate`, `colorHex` (custom building colour), `paintColorIds` (integer array for paint colour notes). Each building row has 3px left accent border, expand/collapse chevron, progress bar, room count, paint colour swatches, drag-to-reorder, and context menu (Add Room, Add Task). Room rows are indented with smaller font, draggable for reorder within their building, and show progress bars. Unsectioned tasks appear as a "General Tasks" virtual room. Dates rendered on Gantt bars (>80px = full date range, 40–80px = start only). Building colour picker popover, paint colour panel with searchable add/remove in task drill-down header. Trade preset chips shown when adding rooms. Drag-to-reorder at building, room, and task levels. Drag timeline bars left/right to shift start/end dates (admin/crew only) with live preview and toast confirmation. Task drill-down has checkboxes to toggle done/to-do, "Move to" context menu for reassigning tasks between rooms. Role-gated: clients see read-only, crew/admin can add buildings/rooms/tasks, reorder, and check off tasks. Schema: milestones table with `colorHex` text and `paintColorIds` integer array columns; sections table stores rooms.
