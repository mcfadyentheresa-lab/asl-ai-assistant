...
*   **Gantt Chart / Progress Tab**: The "Progress" tab in ProjectDetails combines three sub-views: Timeline (Gantt chart), Checklist, and Calendar. The Gantt chart (`client/src/components/GanttChart.tsx`) visualizes project milestones as horizontal timeline bars with progress fill based on associated task completion. Features zoom controls, "Today" marker line, month headers, color-coded phase bars, and a legend. Calendar is no longer a standalone top-level tab — it's accessed within Progress.
