...
*   **Gantt Chart / Progress Tab**: The "Progress" tab in ProjectDetails combines three sub-views: Timeline (Gantt chart), Checklist, and Calendar. The Gantt chart (`client/src/components/GanttChart.tsx`) features a **hierarchical WBS (Work Breakdown Structure)** with three levels: Phase > Section > Task. Phases (milestones) have `startDate`/`endDate` fields. Sections (`sections` table) group tasks under phases with their own date ranges. Tasks optionally link to sections via `sectionId`. The timeline shows collapsible rows with expand/collapse chevrons, phase creation form (moved from Overview to Timeline), inline section creation under phases, and auto-calculated date ranges and progress. Role-gated: clients see read-only, crew/admin can add phases and sections. Phase creation form removed from Overview tab (replaced with hint text directing to Timeline view). Schema: `sections` table (milestoneId FK with cascade delete, projectId, title, startDate, endDate, completed, order). API: `GET/POST /api/projects/:projectId/sections`, `PATCH/DELETE /api/sections/:id` with role-based authorization. Calendar is no longer a standalone top-level tab — it's accessed within Progress.
