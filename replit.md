...
*   **Gantt Chart / Progress Tab (Area-Based Timeline)**: The "Progress" tab in ProjectDetails combines three sub-views: Timeline (Gantt chart), Checklist, and Calendar. The Gantt chart (`client/src/components/GanttChart.tsx`) uses a **progressive drill-down** with three levels: Area → Work Category → Task. UI terminology uses "Area" (mapped to milestones DB table) and "Work Category" (mapped to sections DB table). The left column header reads "AREAS" at top level, "WORK CATEGORIES" when drilled into an area, and "TASKS" when viewing a work category's tasks. Clicking an area row drills into its work categories; clicking a work category drills into its tasks. Full breadcrumb navigation (All Areas > Area > Work Category) allows returning to any parent level. Tasks have checkboxes — checking/unchecking toggles status between "done" and "todo". Progress bars and completion counts update in real time. Each drill level adapts the Gantt timeline date range to the focused scope. Unsectioned tasks under an area appear as a "General Tasks" group. State reconciliation auto-resets drill level when selected area/work category is deleted. Areas (milestones) have `startDate`/`endDate`, `colorHex` (custom area colour), and `paintColorIds` (integer array linking to paintColors table for paint colour notes per area). Dates are rendered directly on Gantt bars when wide enough (>80px = full range, 40-80px = start only). Stronger visual separation with 3px left accent border on each row using the area's colour. Drag-to-reorder rows at area and work category levels using existing `order` field. Trade preset chips (Painting, Electrical, Plumbing, Framing, Drywall, Flooring, Cabinetry, Trim, HVAC, Roofing, Demolition, Insulation, Tiling, Countertops) shown when adding a work category. Area colour picker popover for custom hex colours per area. Paint colour swatches displayed in area rows and as a banner in work category drill-down. Role-gated: clients see read-only, crew/admin can add areas/work categories/tasks, reorder, and check off tasks. Schema: milestones table extended with `colorHex` text and `paintColorIds` integer array columns. Sections table unchanged. API: `PATCH /api/milestones/:id` accepts `colorHex` and `paintColorIds`.
